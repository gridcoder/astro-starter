...
function createBranch(branchName) {
  ...
  try {
    ...
    // Split the output into lines and check if any line contains exactly the branch name (with potential * for current branch)
  ...
  return new Promise((resolve) => {
    ...
    do {
      ...
      try {
        const branches = executeCommand('git branch');
        const branchList = branches.split('\n').map(b => b.trim().replace('* ', ''));
        if (!branchList.includes(newBranchName)) break;
      } catch (error) {
        console.error('Error checking branches:', error.message);
        process.exit(1);
      }
