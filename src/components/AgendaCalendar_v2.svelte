<script>
  ...
  function nextSlide() {
    ...
    currentIndex = (currentIndex + 1) % sortedFutureEvents.length;
  }

  function prevSlide() {
    if (!sortedFutureEvents.length) return;
    currentIndex = (currentIndex - 1 + sortedFutureEvents.length) % sortedFutureEvents.length;
  }
  ...
  // If we're at the end, wrap around to show cards from the beginning
  $: {
    const needed = isMobile ? mobileVisibleCount : desktopVisibleCount;
    if (visibleSlides.length < needed && sortedFutureEvents.length >= needed) {
      const remain = needed - visibleSlides.length;
      visibleSlides = [...visibleSlides, ...sortedFutureEvents.slice(0, remain)];
    }
  }
</script>
